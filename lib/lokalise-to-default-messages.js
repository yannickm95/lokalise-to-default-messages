...
function toDefaultMessages(config) {
  ...
  try {
    ...
    console.log("Failed to parse JSON of translation file:\n", e);
    return;
  }
  ...
  if (config.oldTranslationPath) {
    try {
      ...
      diffDataArray = getDiffKeys(dataArray, ot);
    } catch (e) {
      console.log("Failed to parse JSON of old translation file:\n", e);
      return;
    }
  }

  const data = diffDataArray || dataArray;
...
function getDiffKeys(data, ot) {
  return data.filter(({ key, value }) => !ot[key] || value !== ot[key]);
}
